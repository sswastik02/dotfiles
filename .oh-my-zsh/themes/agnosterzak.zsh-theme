...
prompt_context() {
  if [[ -n "$SSH_CLIENT" ]]; then
    ...
    prompt_segment "#44475a" white "%{$fg_bold[white]%(!.%{%F{white}%}.)%}$@$USER%{$fg_no_bold[white]%}"
  fi
}
...
#«»±˖˗‑‐‒ ━ ✚‐↔←↑↓→↭⇎⇔⋆━◂▸◄►◆☀★☗☊✔✖❮❯⚑⚙
  ...
  if $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
    ...
    if [[ $has_diverged == false && $commits_ahead -gt 0 ]]; then
      if [[ $bgclr == '#6272a4' || $bgclr == 'magenta' ]] then
...
prompt_dir() {
  prompt_segment "#af69ef" white "%{$fg_bold[white]%}%~%{$fg_no_bold[white]%}"
}
...
prompt_time() {
  prompt_segment black white "%{$fg[white]%}at %{$fg[magenta]%}%D{%H:%M:%S}%{$fg_no_bold[magenta]%}%{$fg_no_bold[white]%}"
}

prompt_hostname() {
	prompt_segment "#282a36" white "%{$fg[white]%}on %{$fg[magenta]%}$(hostname)%{$fg_no_bold[magenta]%}%{$fg_no_bold[white]%}"
}
